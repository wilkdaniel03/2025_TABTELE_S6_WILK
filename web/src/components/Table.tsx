...
const Table = (props: ITableProps) => {
	...
	return (
		<Chakra.Table.Root interactive>
			<TableHeader>
			{props.checkable ? <Chakra.Table.ColumnHeader><TableCheckbox checked={checkedList[0]} onCheckedChange={(checked) => onCheckboxChange(0,checked)}/></Chakra.Table.ColumnHeader> : <></>}
			...
			<Chakra.Table.Body>
				{props.data.map((data,index) => {
					return <TableRow key={index} checkable={props.checkable === true} checked={checkedList[index+1]} onCheckedChange={(checked) => onCheckboxChange(index+1,checked)} data={data}/>
				})}
			</Chakra.Table.Body>
		</Chakra.Table.Root>
	);
